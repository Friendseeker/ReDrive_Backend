...
class HardCodedData:
    ...
    ro = City("Romes", apartmentList, "description", parse_url("").url)
    ve = City("Venice", apartmentList, "description", parse_url("").url)
    flo = City("Florence", apartmentList, "description", parse_url("").url)
    mi = City("Milan", apartmentList, "description", parse_url("").url)
    na = City("Naples", apartmentList, "description", parse_url("").url)
    am = City("Amalfi", apartmentList, "description", parse_url("").url)
    tempCitiesItaly = [ro, ve, flo, mi, na, am]
    italyCities = Cities(tempCitiesItaly, Country.IT)

    paris = City("Paris", [], "description", parse_url("").url)
    nice = City("Nice", [], "description", parse_url("").url)
    bordeaux = City("Bordeaux", [], "description", parse_url("").url)
    lyon = City("Lyon", [], "description", parse_url("").url)
    cannes = City("Cannes", [], "description", parse_url("").url)
    corsica = City("Corsica", [], "description", parse_url("").url)
    tempCitiesFrance = [paris, nice, bordeaux, lyon, cannes, corsica]
    franceCities = Cities(tempCitiesFrance, Country.FR)

    tokyo = City("Tokyo", [], "description", parse_url("").url)
    kyoto = City("Kyoto", [], "description", parse_url("").url)
    osaka = City("Osaka", [], "description", parse_url("").url)
    hiroshima = City("Hiroshima", [], "description", parse_url("").url)
    sapporo = City("Sapporo", [], "description", parse_url("").url)
    fukuoka = City("Fukuoka", [], "description", parse_url("").url)
    tempCitiesJapan = [tokyo, kyoto, osaka, hiroshima, sapporo, fukuoka]
    japanCities = Cities(tempCitiesJapan, Country.JP)

    beijing = City("Beijing", [], "description", parse_url("").url)
    shanghai = City("Shanghai", [], "description", parse_url("").url)
    chengdu = City("Chengdu", [], "description", parse_url("").url)
    xian = City("Xi'an", [], "description", parse_url("").url)
    hangzhou = City("Hanzhou", [], "description", parse_url("").url)
